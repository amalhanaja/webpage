<script lang="ts">
	import { Techs } from '$lib/models/Tech';
	import IlWhatIDo from '../Illustrations/IlWhatIDo.svelte';
	import SectionHeader from '../SectionHeader.svelte';
	import Tech from '../Tech.svelte';
</script>

<section class="py-8">
	<SectionHeader title="What I do" />
	<h3 class="mt-4 font-medium">
		Creative Android developer who loves to experiment with new technologies.
	</h3>

	<div class="my-8 flex flex-col sm:flex-row gap-y-8 gap-x-8">
		<section class="techs inline-flex flex-wrap justify-center items-baseline gap-x-2 gap-y-[0.5rem] sm:gap-y-0 sm:gap-x-3">
			{#each Techs as t}
				<Tech tech={t} />
			{/each}
		</section>
		<section class="inline-flex flex-col gap-8">
			<figure class="hidden sm:inline">
				<IlWhatIDo />
			</figure>
			<article class="ml-4">
				<ul class="list-disc">
					<li>Build high-quality Android / Web Applications.</li>
					<li>Work collaboratively with team members.</li>
					<li>Stay up-to-date on the latest technologies and trends.</li>
					<li>Deliver high-quality results on time and within budget.</li>
					<li>Integration of third party services such as Firebase, Supabase, Mixpanel, etc.</li>
				</ul>
			</article>
		</section>
	</div>
</section>

<style>
</style>
